...
- (void)viewDidLoad {
    ...
    // 初始化 FaceUnity 美颜等参数
    UIButton *faceUnityButton = [[UIButton alloc] init];
    [faceUnityButton setImage:[UIImage imageNamed:@"qn_sticker"] forState:(UIControlStateNormal)];
    [faceUnityButton sizeToFit];
    [faceUnityButton addTarget:self action:@selector(clickFaceUnityButton:) forControlEvents:(UIControlEventTouchUpInside)];
    [self.view addSubview:faceUnityButton];
    [faceUnityButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerY.equalTo(self.recordButton);
        make.right.equalTo(self.recordButton.mas_left).offset(-50);
...
- (void)clickDismissPickerViewButton:(UIButton *)button {
    ...
    if (self.faceUnityView.frame.origin.y < self.view.bounds.size.height) {
        CGRect rc = self.faceUnityView.frame;
        [self.faceUnityView mas_remakeConstraints:^(MASConstraintMaker *make) {
...
- (void)faceUnityView:(QNFaceUnityView *)faceUnityView showTipString:(NSString *)tipString {
    ...
    [UIView animateWithDuration:.3 animations:^{
        self.faceUnityTipLabel.alpha = 1;
    }];
...
- (void)hideTipLabel {
    [UIView animateWithDuration:.3 animations:^{
        self.faceUnityTipLabel.alpha = 0;
